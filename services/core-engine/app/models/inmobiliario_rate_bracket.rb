...
class InmobiliarioRateBracket < ActiveRecord::Base
  ...
  validates :tax_type, :year, :base_from, :base_to, :rate_pct, :minimum_amount, :position, presence: true
  validates :base_to, numericality: { greater_than_or_equal_to: :base_from }, if: :base_from
